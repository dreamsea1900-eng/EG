...
> 與 `.claude/skills/design-tokens-reference/SKILL.md` 配合使用：該 skill 說明何時／為何需要查這份資料，本檔案提供實際數值。
...
## Text Styles（Figma Style ↔ Tailwind Class 對照）

> 已建立於 Figma 檔案 `ffElLQAdQgdYalwkic9RjG`（命名 `Text/H2/Bold` ~ `Text/H8/Regular`）。
> 依 `web-casino-eg` 程式碼中 `text-brand-h*` + `font-*` 實際使用頻率整理，字型家族統一為 Inter。
> 原始整理過程記錄於 `figma-color-styles-todo.md`（Color Style 部分，待 `Button/Disabled` token 解決後一併併入本檔案）。
...
設計稿上的文字圖層套用對應 Text Style 後，狀態二產出 HTML 時直接對應到上表的 Tailwind Class。
